...
const App: React.FC = () => {
  ...
  const handleDownload = async () => {
    // Download what's currently visible
  ...
  return (
    <div className="relative h-screen w-screen bg-black overflow-hidden select-none text-white flex flex-col">
      <input 
        type="file" 
        ref={fileInputRef} 
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden px-4 pt-4 pb-44">
        {capturedImage ? (
          <>
            <img 
            ...
            />
            <div className={`relative z-10 w-full h-full flex items-center justify-center transition-all duration-1000 ${isGenerating ? 'blur-3xl opacity-10 scale-95' : 'blur-0 opacity-100 scale-100'}`}>
              <img 
                src={showOriginal ? capturedImage : (resultImage || capturedImage)} 
                alt="Display" 
          ...
          <div className="flex flex-col items-center animate-in fade-in duration-1000">
            ...
            <p className="mt-4 text-white/10 text-[10px] font-black uppercase tracking-[0.4em]">Banana Creative Lab</p>
          </div>
        )}
      </div>
      ...
      {/* Secondary Actions Row (Settings, Compare, Download) */}
      <div className="absolute bottom-36 left-0 right-0 flex justify-center items-center gap-6 z-30 pointer-events-none px-6">
        <button 
          ...
          className="pointer-events-auto w-12 h-12 flex items-center justify-center rounded-2xl bg-black/40 border border-white/10 backdrop-blur-xl active:scale-90 transition-all shadow-xl"
        >
        ...
        <button 
          ...
          className={`pointer-events-auto w-16 h-16 rounded-[1.8rem] backdrop-blur-3xl border flex items-center justify-center shadow-2xl active:scale-90 transition-all ${showOriginal ? 'bg-blue-600 border-blue-400 text-white' : 'bg-white/5 border-white/10 text-white/60'} disabled:opacity-10`}
        >
        ...
        <button 
          ...
          disabled={!capturedImage || isGenerating}
          className="pointer-events-auto w-12 h-12 flex items-center justify-center rounded-2xl bg-white text-black active:scale-90 transition-all shadow-xl disabled:opacity-20"
        >
      ...
      {/* Main Control Bar */}
      <div className="absolute bottom-10 left-6 right-6 z-50 flex items-center justify-between gap-3 h-20 px-4 bg-black/70 border border-white/10 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl animate-in slide-in-from-bottom-10 duration-700">
        <button 
